...
async function generateMockDiagnosis(clientId: string) {
    ...
    const bioAnalysis = `A bio do perfil deve comunicar com clareza quem é a marca, o que ela oferece e qual é o próximo passo esperado do visitante. Para ${client.name}, é importante reforçar a proposta de valor de forma direta, evitando excesso de informações soltas. ${instagramUrl ? `Perfil analisado: ${instagramUrl}` : "Nenhum link de Instagram foi informado ainda."
        }`;

    const profilePhotoAnalysis = `A foto de perfil precisa ser legível mesmo em tamanho pequeno. O ideal é usar uma marca, símbolo ou imagem com bom contraste, sem excesso de detalhes, para facilitar o reconhecimento rápido do perfil.`;
    ...
    const highlightsAnalysis = `Os destaques precisam funcionar como uma vitrine rápida. Recomenda-se organizar informações essenciais como serviços, depoimentos, bastidores, dúvidas frequentes, localização, resultados e formas de contato.`;
    ...
    const offerClarityAnalysis = `A oferta precisa deixar claro o que a empresa vende, para quem vende e por que o público deveria escolher essa marca. Conteúdos devem reduzir dúvidas, aumentar confiança e conduzir o público para uma ação.`;
    ...
    const strengths = [
        ...
        client.contractedServices
            ? `Serviços contratados já documentados: ${client.contractedServices}.`
            : "Há espaço para documentar melhor os serviços trabalhados.",
        persona?.desires
            ? `A persona possui desejos mapeados: ${persona.desires}.`
